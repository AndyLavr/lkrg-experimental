...
char p_sys_tkill_kretprobe_state = 0x0;

static struct kretprobe p_sys_tkill_kretprobe = {
    .kp.symbol_name = "sys_tkill",
    .handler = p_sys_tkill_ret,
    .entry_handler = p_sys_tkill_entry,
    .data_size = sizeof(struct p_sys_tkill_data),
...
// STRONG_DEBUG
   ...
   if (p_is_protected_pid(p_regs->di)) {
      p_print_log(P_LKRG_INFO,
             "Normal process[%s] pid:%d tries to tkill() protected "
             "pid:%ld with sign:%ld\n",
             current->comm,current->pid,p_regs->di,p_regs->si);
      p_regs->di = -1;
      p_regs->si = -1;
...
// STRONG_DEBUG
   ...
   if ( (p_ret = register_kretprobe(&p_sys_tkill_kretprobe)) < 0) {
      ...
      goto p_install_sys_tkill_hook_out;
   }
   p_print_log(P_LKRG_INFO, "Planted [kretprobe] <%s> at: %p\n",
                     p_sys_tkill_kretprobe.kp.symbol_name, p_sys_tkill_kretprobe.kp.addr);
   p_sys_tkill_kretprobe_state = 0x1;
...
// STRONG_DEBUG
   ...
   if (!p_sys_tkill_kretprobe_state) {
      ...
      unregister_kretprobe(&p_sys_tkill_kretprobe);
      p_print_log(P_LKRG_INFO, "Removing [kretprobe] <%s> at 0x%p\n",
                     p_sys_tkill_kretprobe.kp.symbol_name, p_sys_tkill_kretprobe.kp.addr);
      p_sys_tkill_kretprobe_state = 0x0;
   }
