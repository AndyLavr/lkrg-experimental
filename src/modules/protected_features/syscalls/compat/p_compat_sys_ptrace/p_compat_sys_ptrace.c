...
char p_compat_sys_ptrace_kretprobe_state = 0x0;

static struct kretprobe p_compat_sys_ptrace_kretprobe = {
    .kp.symbol_name = "compat_sys_ptrace",
...
int p_compat_sys_ptrace_entry(struct kretprobe_instance *p_ri, struct pt_regs *p_regs) {
   ...
   if (p_is_protected_pid(current->pid)) {
      goto p_compat_sys_ptrace_entry_out;
   }
...
// STRONG_DEBUG
   p_debug_log(P_LKRG_STRONG_DBG,
          "Entering function <p_install_compat_sys_ptrace_hook>\n");

   if ( (p_ret = register_kretprobe(&p_compat_sys_ptrace_kretprobe)) < 0) {
      p_print_log(P_LKRG_ERR, "[kretprobe] register_kretprobe() failed! [err=%d]\n",p_ret);
      goto p_install_compat_sys_ptrace_hook_out;
   }
   p_print_log(P_LKRG_INFO, "Planted [kretprobe] <%s> at: %p\n",
                     p_compat_sys_ptrace_kretprobe.kp.symbol_name, p_compat_sys_ptrace_kretprobe.kp.addr);
   p_compat_sys_ptrace_kretprobe_state = 0x1;

//   p_ret = 0x0; <- should be 0x0 anyway...

p_install_compat_sys_ptrace_hook_out:

// STRONG_DEBUG
   p_debug_log(P_LKRG_STRONG_DBG,
          "Leaving function <p_install_compat_sys_ptrace_hook> (p_ret => %d)\n",p_ret);
...
// STRONG_DEBUG
   p_debug_log(P_LKRG_STRONG_DBG,
          "Entering function <p_uninstall_compat_sys_ptrace_hook>\n");

   if (!p_compat_sys_ptrace_kretprobe_state) {
      p_print_log(P_LKRG_INFO, "[kretprobe] <%s> at 0x%p is NOT installed\n",
                     p_compat_sys_ptrace_kretprobe.kp.symbol_name, p_compat_sys_ptrace_kretprobe.kp.addr);
   } else {
      unregister_kretprobe(&p_compat_sys_ptrace_kretprobe);
      p_print_log(P_LKRG_INFO, "Removing [kretprobe] <%s> at 0x%p\n",
                     p_compat_sys_ptrace_kretprobe.kp.symbol_name, p_compat_sys_ptrace_kretprobe.kp.addr);
      p_compat_sys_ptrace_kretprobe_state = 0x0;
   }

// STRONG_DEBUG
   p_debug_log(P_LKRG_STRONG_DBG,
          "Leaving function <p_uninstall_compat_sys_ptrace_hook>\n");
}
