 ...
 *
 * Component:
 *  - Intercept compat_execve syscall
 *
 * Notes:
...
char p_compat_sys_execve_kretprobe_state = 0x0;

static struct kretprobe p_compat_sys_execve_kretprobe = {
    .kp.symbol_name = "compat_sys_execve",
    .handler = p_compat_sys_execve_ret,
    .entry_handler = p_compat_sys_execve_entry,
    .data_size = sizeof(struct p_compat_sys_execve_data),
...
int p_compat_sys_execve_ret(struct kretprobe_instance *ri, struct pt_regs *p_regs) {
...
//   p_print_log(P_LKRG_CRIT,
          "p_compat_sys_execve_ret: returned value => %ld comm[%s] Pid:%d inode[%ld]\n",
...
int p_install_compat_sys_execve_hook(void) {

   int p_ret;

   p_debug_log(P_LKRG_STRONG_DBG,
          "Entering function <p_install_compat_sys_execve_hook>\n");

   if ( (p_ret = register_kretprobe(&p_compat_sys_execve_kretprobe)) < 0) {
      p_print_log(P_LKRG_ERR, "[kretprobe] register_kretprobe() failed! [err=%d]\n",p_ret);
      goto p_uninstall_compat_sys_execve_hook_out;
   }
   p_print_log(P_LKRG_INFO, "Planted [kretprobe] <%s> at: %p\n",
                     p_compat_sys_execve_kretprobe.kp.symbol_name, p_compat_sys_execve_kretprobe.kp.addr);
   p_compat_sys_execve_kretprobe_state = 0x1;

//   p_ret = 0x0; <- should be 0x0 anyway...

p_uninstall_compat_sys_execve_hook_out:

   p_debug_log(P_LKRG_STRONG_DBG,
          "Leaving function <p_install_compat_sys_execve_hook> (p_ret => %d)\n",p_ret);

   return p_ret;
}


void p_uninstall_compat_sys_execve_hook(void) {

   p_debug_log(P_LKRG_STRONG_DBG,
          "Entering function <p_uninstall_compat_sys_execve_hook>\n");

   if (!p_compat_sys_execve_kretprobe_state) {
      p_print_log(P_LKRG_INFO, "[kretprobe] <%s> at 0x%p is NOT installed\n",
                     p_compat_sys_execve_kretprobe.kp.symbol_name ,p_compat_sys_execve_kretprobe.kp.addr);
   } else {
      unregister_kretprobe(&p_compat_sys_execve_kretprobe);
      p_print_log(P_LKRG_INFO, "Removing [kretprobe] <%s> at 0x%p\n",
                     p_compat_sys_execve_kretprobe.kp.symbol_name ,p_compat_sys_execve_kretprobe.kp.addr);
      p_compat_sys_execve_kretprobe_state = 0x0;
   }

   p_debug_log(P_LKRG_STRONG_DBG,
          "Leaving function <p_uninstall_compat_sys_execve_hook>\n");
}
